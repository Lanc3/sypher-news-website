    ...
    <main className="flex-1 py-10 sm:py-14">
      <SiteContainer max="md">
        <header className="panel px-5 py-6 sm:px-8 sm:py-8">
          <p className="font-mono text-[10px] font-medium uppercase tracking-[0.35em] text-[#ff2bd6] sm:text-xs">Index</p>
          <h1 className="mt-2 font-mono text-2xl font-bold tracking-tight text-[#00ff41] sm:text-3xl">/news</h1>
          <p className="mt-3 max-w-xl text-sm leading-relaxed text-[#9a9a9a] sm:text-base">
            Browse channels. Articles resolve at{" "}
            <code className="rounded border border-[#00ff41]/25 bg-black/50 px-1.5 py-0.5 text-xs text-[#00ff41]/90">
              /news/[category]/[slug]
            </code>
            .
          </p>
        ...
        <ul className="mt-8 grid gap-3 sm:mt-10 sm:grid-cols-2 sm:gap-4">
          {categories.map((c) => (
            <li key={c.id}>
              ...
              >
                <span className="font-mono text-base font-semibold text-[#ff2bd6] sm:text-lg">{c.name}</span>
